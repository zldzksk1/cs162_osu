...
//game play or not
	...
	else if (x == 2)
	{
		while (!validate)
		{
			...
			if (cin.fail() || userNum < 1 || userNum > 2000000 || cin.get() != '\n')						//if the input is not an integer, or unintended interger
			{
				cout << "Your input is wrong, too large or too small. please try again." << endl << endl;	//print error message
				cin.clear();																				//clear the cin
				cin.ignore(256, '\n');																		//skip to the newline
			}
	...
	else if (x == 4)
	{
		while (!validate)
		{
			...
			if (cin.fail() || userNum < -2000000 || userNum > 2000000 || cin.get() != '\n')						//if the input is not an integer, or unintended interger
			{
				cout << "Your input is wrong, too large or too small. please try again." << endl << endl;	//print error message
				cin.clear();																				//clear the cin
				cin.ignore(256, '\n');																		//skip to the newline
